defmodule ThySupervisorOld do
  ...
  def start_link(child_spec_list) do
      GenServer.start_link(__MODULE__, [child_spec_list]) # expects ThySupervisor.init/1
  end
  ...
  def init([child_spec_list]) do
      Process.flag(:trap_exit, true) # makes the spv pr trap exits
  ...
  def handle_info({:EXIT, from, :killed}, state) do #handle all :exit msgs
  ...
  def handle_info({:EXIT, old_pid, _reason}, state) do #Restarting a child process that exits abnormally
    case HashDict.fetch(state, old_pid) do
        {:ok, child_spec} ->
            case restart_child(old_pid, child_spec) do
                {:ok, {pid, child_spec}} ->
  ...
  # updates the supervisor state by removing its entry in the HashDict
  ...
  defp start_children([child_spec|rest]) do
      case start_child(child_spec) do
          {:ok, pid} ->
              [{pid, child_spec} | start_children(rest)] #return tuple with pid and its specs
          :error ->
              :error
      end
  end
